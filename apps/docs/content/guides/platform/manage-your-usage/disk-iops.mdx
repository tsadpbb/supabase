...
Each database has a dedicated disk. You are charged for the provisioned disk IOPS. You can configure disk IOPS in the advanced settings on the [project's Compute and Disk page](https://supabase.com/dashboard/project/_/settings/compute-and-disk).
...
Disk IOPS is charged by IOPS-Hrs. 1 IOPS-Hrs represents 1 IOPS being provisioned for 1 hour. For example, having 10 IOPS provisioned for 5 hours results in 50 IOPS-Hrs (10 IOPS × 5 hours).
...
$0.00003288 per IOPS-Hrs ($0.024 per IOPS per month). gp3 disks come with a default IOPS of 3,000. You are only charged for provisioned IOPS exceeding these 3,000 IOPS.
...
$0.000163 per IOPS-Hrs ($0.119 per IOPS per month).
