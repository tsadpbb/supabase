...
Each database has a dedicated disk. You are charged for the provisioned disk throughput. You can configure disk throughput in the advanced settings on the [project's Compute and Disk page](https://supabase.com/dashboard/project/_/settings/compute-and-disk).
...
Disk throughput is charged by Mbps-Hrs (Mbps stands for Megabits per second). 1 Mbps-Hrs represents disk throughput of 1 Mbps being provisioned for 1 hour. For example, having 10 Mbps provisioned for 5 hours results in 50 Mbps-Hrs (10 Mbps × 5 hours).
...
$0.00013 per Mbps-Hrs ($0.095 per Mbps per month). gp3 disks come with a baseline throughput of 125 Mbps. You are only charged for provisioned throughput exceeding these 125 Mbps.

<Admonition type="note">
  Launching a Read Replica creates an additional database with its own dedicated disk. Read Replicas inherit any additional provisioned disk throughput from the primary database. You are charged for this configured throughput.
</Admonition>
